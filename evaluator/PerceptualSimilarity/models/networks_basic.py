...
from evaluator.PerceptualSimilarity.models import ps_util as m_util
...
class PNetLin(nn.Module):
    ...
    def forward(self, in0, in1, retPerLayer=False):
        ...
        for kk in range(self.L):
            feats0[kk], feats1[kk] = m_util.normalize_tensor(
                outs0[kk]), m_util.normalize_tensor(outs1[kk])
...
class L2(FakeNet):
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if (self.colorspace == 'RGB'):
            (N, C, X, Y) = in0.size()
            value = torch.mean(torch.mean(
                torch.mean((in0 - in1) ** 2, dim=1).view(N, 1, X, Y),
                dim=2).view(N, 1, 1, Y), dim=3).view(N)
            return value
        elif (self.colorspace == 'Lab'):
            value = m_util.l2(m_util.tensor2np(
                m_util.tensor2tensorlab(in0.data, to_norm=False)),
                              m_util.tensor2np(
                                  m_util.tensor2tensorlab(in1.data,
                                                          to_norm=False)),
...
class DSSIM(FakeNet):
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if (self.colorspace == 'RGB'):
            value = m_util.dssim(1. * m_util.tensor2im(in0.data),
                                 1. * m_util.tensor2im(in1.data),
                                 range=255.).astype('float')
        elif (self.colorspace == 'Lab'):
            value = m_util.dssim(m_util.tensor2np(
                m_util.tensor2tensorlab(in0.data, to_norm=False)),
                                 m_util.tensor2np(
                                     m_util.tensor2tensorlab(in1.data,
                                                             to_norm=False)),
