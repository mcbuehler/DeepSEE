...
parser.add_argument('--display_port', type=int, default=8001,
...
# initialize model
model = dm.DistModel()
model.initialize(model=opt.model, net=opt.net, use_gpu=opt.use_gpu,
                 is_train=True,
    ...
    for i, data in enumerate(dataset):
        ...
        if total_steps % opt.print_freq == 0:
            ...
            visualizer.print_current_errors(epoch, epoch_iter, errors, t,
                                            t2=t2, t2o=t2o, fid=fid)

            for key in errors.keys():
                visualizer.plot_current_errors_save(epoch, float(
                    epoch_iter) / dataset_size, opt, errors, keys=[key, ],
                                                    name=key,
